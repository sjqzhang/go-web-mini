...
type {{.Table.TableName}} struct {
    Model
    {{range .Fields}}{{if  checkField .ColumnName}}{{.CamelField}} {{.RealType}} `gorm:"{{.ColumnName}};type:{{.ColumnType}};comment:'{{.ColumnCommentForView}}'" json:"{{.ColumnName}}"{{.KeyStr }}` {{.ColumnComment}}
    {{end}}{{end}}
}
