...
type Menu struct {
	...
	Path       string  `gorm:"uniqueIndex:uniq_path_component;type:varchar(100);comment:'菜单访问路径'" json:"path"`
	Redirect   *string `gorm:"type:varchar(100);comment:'重定向路径'" json:"redirect"`
	Component  string  `gorm:"uniqueIndex:uniq_path_component;type:varchar(100);comment:'前端组件路径'" json:"component"`
